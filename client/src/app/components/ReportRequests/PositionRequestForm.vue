...
EXAMPLE: Looking for a tall target man. Must be strong enough to hold off defenders. The player will be the key fulcrum in the transition from defence to attack. Decent heading ability to complement their height and strength. Would also prefer players that have a good first touch.
      ...
      <div class="col-md-12">
        ...
        <div class="row">
          <div class="col-lg-4 row form-group">
            ...
            <input type="number" min="0" class="col-lg-5 form-control" v-model.number="meta_data.min_age" placeholder="Min age" v-validate="'between:16,40|min_range:' + meta_data.max_age" name="age">
            <span class="separator col-lg-1">-</span>
            <input type="number" min="0" class="col-lg-5 form-control" v-model.number="meta_data.max_age" v-validate="'between:16,40|max_range:' + meta_data.min_age" name="age" placeholder="Max age">
          </div>
          <div class="col-lg-4 row form-group">
            <label class="col-md-12">Select Height (cm)</label>
            <input type="number" min="0" class="col-lg-5 form-control" v-model.number="meta_data.min_height" placeholder="Min height" v-validate="'between:160,200|min_range:' + meta_data.max_height" name="height">
            <span class="separator col-lg-1">-</span>
            <input type="number" min="0" class="col-lg-5 form-control" v-model.number="meta_data.max_height" placeholder="Max height" v-validate="'between:160,200|max_range:' + meta_data.min_height" name="height">
          </div>
          <div class="col-lg-4 row form-group">
            <label class="col-md-12">Select Weight (kg)</label>
            <input type="number" min="0" class="col-lg-5 form-control" v-model.number="meta_data.min_weight" placeholder="Min weight" v-validate="'between:60,100|min_range:' + meta_data.max_weight" name="weight">
            <span class="separator col-lg-1">-</span>
            <input type="number" min="0" class="col-lg-5 form-control" v-model.number="meta_data.max_weight" placeholder="Max weight" v-validate="'between:60,100|max_range:' + meta_data.min_weight" name="weight">
          </div>
        </div>
      </div>
      <div class="row">
      ...
      <div class="row col-lg-12 form-group required-before">
        <ft-datepicker class="col-lg-12 form-control" :disabledDates="disabledDates" :value="deadline" v-on:update:val="deadline = $event" placeholder="Select a deadline" />
